...
따라서 $\mathcal{N}_{x} = \{ x_i \in \mathcal{P} \ | \left\| x_i-x \right\| \lt r  \} \ \ \cdots \ \  (1)  $<br><br>
...
g는 $\mathcal{B}_{r}^{3} = \{ y \in \mathbb{R}^{3} | \left\| y \right\| \lt r \}$ 범위로 정의된다.<br><br>
...
- Kernel point : $\{ \tilde{x}_{k} | k < K  \} \in \mathcal{B}_{r}^{3}$<br>
- 특성을 차원 Din에서 Dout로 매핑하는 것에 관련된 가중치 행렬 : $\{ W_k | k < K \} \in \mathbb{R}^{D_{in} \times D_{out}}$<br>
...
$ h(y_i,\tilde{x}_{k})=max(0, \ 1-\frac{\left\| y_i-\tilde{x}_{k} \right\|}{\sigma}) $<br><br>
