...
-> Asymmetric edge function<br>
$h_{\Theta}(X_i,X_j)=\bar{h}_{\Theta}(X_i,X_j-X_i)$<br><br>
이는 patch center $X_i$의 좌표에 의해 포착된 전역 모양 구조와 $X_j-X_i$에 의해 포착된 지역 이웃 정보를 명시적으로 결합한다.<br><br>
본 논문의 operator는 다음과 같이 정의한다.<br>
$e_{ijm}'=ReLU(\theta_{m}\cdot (X_j-X_i)+\phi_{m} \cdot X_i)$<br>
$where, \ \ \Theta=(\theta_1,...,\theta_m,\phi_1,...,\phi_m)$
<br><br>
shared MLP로 구현될 수 있으며, 다음으로 아래 식을 수행한다.<br>
$x_{im}'= \underset{j:(i,j)\in\mathcal{E}}{max}e_{ijm}'$<br><br>
